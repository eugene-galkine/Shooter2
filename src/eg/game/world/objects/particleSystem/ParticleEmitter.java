...
{
	float emitDuration, minDuration, maxDuration, minDir, maxDir, minSpeed, maxSpeed, minSize, maxSize, x, y, dir;
	int minObjs, maxObjs;
	...
	public ParticleEmitter (int minObjs, int maxObjs, float emitDuration, float minDuration, float maxDuration, float minDir, float maxDir,
			float minSpeed, float maxSpeed, float minSize, float maxSize, float x, float y, float dir)
	{
	...
	private void emit()
	{
		...
		for (int i = 0; i < count; i++)
			GameWorld.getInstance().addObject(new Particle(x, y, dir + minDir + (r.nextFloat() * (maxDir - minDir)), minSpeed  + (r.nextFloat() * (maxSpeed - minSpeed)), minDuration + (r.nextFloat() * (maxDuration - minDuration)), minSize + (r.nextFloat() * (maxSize - minSize))));
	}
}
